...
<div class="layui-card layui-bg-gray"><style>        .layui-tab-card>.layui-tab-title .layui-this {
    ...
    ?><fieldset><legend>数据统计</legend><font color="red">交易总额：<?php echo htmlentities($order_sum); ?>元</font> &nbsp;&nbsp;&nbsp;<font color="red">佣金总额：<?php echo htmlentities($commission_sum); ?>元</font></fieldset><table class="layui-table margin-top-15" lay-skin="line"><?php if(!(empty($list) || (($list instanceof \think\Collection || $list instanceof \think\Paginator ) && $list->isEmpty()))): ?><thead><tr><th class='text-left nowrap'>订单号</th><th class='text-left nowrap'>真实姓名</th><th class='text-left nowrap'>会员等级</th><th class='text-left nowrap'>余额</th><th class='text-left nowrap'>今日单数</th><th class='text-left nowrap'>单价数量</th><th class='text-left nowrap'>总价佣金</th><th class='text-left nowrap'>下单时间 解冻时间</th><th class='text-left nowrap'>交易状态</th><th class='text-left nowrap'>物品质量</th><th class='text-left nowrap'>物品服务</th><th class='text-left nowrap'>态度服务</th><th class='text-left nowrap'>传买家秀</th><th class='text-left nowrap'>操作</th><th class='text-left nowrap'>商品名称</th></tr></thead><?php endif; ?><tbody><?php foreach($list as $key=>$vo): ?><tr><td class='text-left nowrap J_id'><?php echo htmlentities($vo['id']); ?></td><td class='text-left nowrap'><?php echo htmlentities($vo['username']); ?></td><td class='text-left nowrap'><?php echo model('Users')->getLevel($vo['level'],'name');; ?></td><td class='text-left nowrap'><?php echo htmlentities($vo['ubalance']); ?></td><td class='text-left nowrap '><?php echo htmlentities(format_datetime($vo['addtime'],'m-d')); ?><p class="J_today_num">排序</p></td><td class='text-left nowrap'>单价&nbsp;$<?php echo htmlentities($vo['goods_price']); ?><p>数量:&nbsp; <?php echo htmlentities($vo['goods_count']); ?></td><td class='text-left nowrap'><font color="red">总价&nbsp;$<?php echo htmlentities($vo['num']); ?></font><p>佣金&nbsp;$<?php echo htmlentities($vo['commission']); ?></td><td class='text-left nowrap'><?php echo htmlentities(format_datetime($vo['addtime'])); ?><p><?php echo htmlentities(format_datetime($vo['endtime'])); ?></td><td class='text-left nowrap'><?php echo htmlentities($vo['good_quality']); ?></td><td class='text-left nowrap'><?php echo htmlentities($vo['good_service']); ?></td><td class='text-left nowrap'><?php echo htmlentities($vo['attitude_service']); ?></td><td class='text-left nowrap'><a data-title="查看图片" data-modal='<?php echo url("admin/users/picinfo"); ?>?pic=<?php echo htmlentities($vo['buyer_show_img']); ?>'><img src="<?php echo htmlentities($vo['buyer_show_img']); ?>" style="width:150px;height:100px;"></a></td><td class='text-left nowrap' style="color: red;"><?php switch($vo['status']): case "0": ?>                    等待付款
                    
                    <?php break; case "1": ?>已付款<?php break; case "2": ?>待发货<?php break; case "3": ?>已发货<?php break; case "4": ?>待收货<?php break; case "5": ?>已收货<?php break; case "6": ?>待评价<?php break; case "7": ?>评价完成<?php break; ?><?php endswitch; ?></td><td class='text-left nowrap'><?php if($vo['status']==2): ?><!-- <a data-csrf="<?php echo systoken('admin/deal/do_user_order'); ?>" class="layui-btn layui-btn-xs layui-btn"
                        data-action="<?php echo url('do_user_order'); ?>" data-value="id#<?php echo htmlentities($vo['id']); ?>;status#4">发货</a> --><!-- <a data-csrf="<?php echo systoken('admin/deal/do_user_order'); ?>" class="layui-btn layui-btn-xs layui-btn-warm"
                        data-action="<?php echo url('do_user_order'); ?>" data-value="id#<?php echo htmlentities($vo['id']); ?>;status#4">取消订单</a> --><a data-dbclick class="layui-btn layui-btn-xs" data-title="发货"
                        data-modal='<?php echo url("admin/deal/shipping"); ?>?id=<?php echo htmlentities($vo['id']); ?>'>发货</a><?php endif; ?></td><td class='text-left nowrap'><?php echo htmlentities($vo['goods_name']); ?></td></tr><?php endforeach; ?></tbody></table><?php if(empty($list) || (($list instanceof \think\Collection || $list instanceof \think\Paginator ) && $list->isEmpty())): ?><span class="notdata">没有记录哦</span><?php else: ?><?php echo (isset($pagehtml) && ($pagehtml !== '')?$pagehtml:''); ?><?php endif; ?></div><script type="text/javascript" charset="utf-8">    $(function () {
        $(".J_id").each(function () {
            var id = $(this).text();
            console.log(id);
            var _this = $(this);
