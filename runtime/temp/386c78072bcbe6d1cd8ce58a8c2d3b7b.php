...
<div class="layui-card layui-bg-gray"><style>        .layui-tab-card>.layui-tab-title .layui-this {
    ...
    layui.use('slider', function(){
      ...
      //开启范围选择
      slider.render({
        elem: '#deal_rulebaifenbi'
        ,value: [min,max] //初始值
        ,range: true //范围选择
        ,change: function(vals){
          $('#test-slider-tips2baifenbi').html('匹配范围：'+ vals[0] + '% ~ '+ vals[1]+'%');
          $('#minbaifenbi').val(vals[0]);
          $('#maxbaifenbi').val(vals[1]);
        }
      });      
    });
