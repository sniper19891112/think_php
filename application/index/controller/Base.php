...
class Base extends Controller
{
    ...
    public function check_deal()
    {
        ...
        if ($uinfo['status'] == 2) {
            return ['code' => 1, 'info' => lang('该账户已被禁用')];
        }

        if ($uinfo['deal_status'] == 0) {
            return ['code' => 1, 'info' => lang('该账户交易功能已被冻结')];
        }
        ...
        if ($uinfo['balance'] < config('deal_min_balance')) {
            return ['code' => 1, 'info' => lang('余额低于') . config('deal_min_balance') . lang('无法继续交易')];
        }
