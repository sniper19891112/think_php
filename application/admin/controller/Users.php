                ...
                foreach ($data as &$datum) {
                    ...
                    switch ($datum['type']) {
                        ...
                        case 7:
                            ...
                            break;
                        case 11:
                            $text = '<span class="layui-btn layui-btn-sm layui-btn-danger">充值彩金</span>';
                            break;
                        default:
                            $text = '其他';
                    }
...
//            $show_td = input('post.show_td/d',0);  //显示我的团队
            ...
            if ($kouchu_balance < 0 && $userinfo['balance'] < abs($kouchu_balance)) {
                $this->error('扣除金额额大于用户余额');
            }
    ...
    public function edit_users_balancebk()
    {
        ...
        if (request()->isPost()) {
            ...
            if (!$userinfo) {
                $this->error('为查询到用户参数错误');
            }

            if ($balance < 0 && $userinfo['balance'] < abs($balance)) {
                $this->error('扣除金额额大于用户余额');
            }
