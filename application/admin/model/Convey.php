...
class Convey extends Model
{
    ...
    {
        $add_id = db('xy_member_address')->where('uid', $uid)->value('id'); //获取收款地址信息s
        if (!$add_id) {
            return ['code' => 1, 'info' => lang('还没有设置收货地址')];
        }

        $uinfo = db('xy_users')->field('deal_status,balance,level,deal_min_num,deal_max_num,pipei_type,pipei_grouping')->find($uid);
        if ($uinfo['deal_status'] != 2) {
            return ['code' => 1, 'info' => lang('抢单已终止')];
        }

        $cidinfo = db('xy_goods_cate')->field('deal_min_num,deal_max_num')->find($cid); //获取收款地址信息s
        ...
        if (($uinfo['pipei_type'] == 2) && ($uinfo['pipei_grouping'] > 0)) {
            ...
            if ($group_pipei['cancontinue'] == 0) {
                return ['code' => 1, 'info' => lang('如需继续刷单请联系派单员')];
            }
...
//return ['code'=>8,'info'=>'第'.$di_num_dan.'单;区间min:'.$min.'max:'.$max.'总价'.$goods['num'].'数量'.$goods['count'].'单价'.$goods['price'].'利润:'.$commission];
        $level = $uinfo['level'];
        !$uinfo['level'] ? $level = 0 : '';
        $ulevel = db('xy_level')->where('level', $level)->find();
        ...
        $id = getSn('UB');
        Db::startTrans();
        $res = db('xy_users')->where('id', $uid)->update(['deal_status' => 3, 'deal_time' => strtotime(date('Y-m-d')), 'deal_count' => Db::raw('deal_count+1')]); //将账户状态改为交易中
        //通过商品id查找 佣金比例
        //$cate = db('xy_goods_cate')->find($goods['cid']);
    ...
    {
        $info = db('xy_convey')->find($oid);
        if (!$info) {
            return ['code' => 1, 'info' => lang('订单号不存在')];
        }
