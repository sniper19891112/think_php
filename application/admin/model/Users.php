...
class Users extends Model
{
     ...
     * 重置密码
     */
    public function reset_pwd($username, $pwd, $type = 1)
    {
        $data = [
            'username' => $username,
            'pwd' => $pwd,
        ...
        $user_id = Db::table($this->table)->where(['username' => $username])->value('id');
        if (!$user_id) {
    ...
    public function getLevel($level, $field)
    {
        if (!$level) {
            $level = 0;
        }
    ...
    {
        if (!$pid || $pid <= 0 || strlen($pid) <= 0 || !in_array($find_column, array('id', 'parent_id'))) {
            return 0;
        }
    ...
    public function disbursement($oid)
    {
        ...
        if ($r) {
            ...
            if (!$bankinfo['bankcode']) {
                return ['code' => 0, 'info' => 'bankcode为空'];
            }

            if (!$bankinfo['username']) {
                return ['code' => 0, 'info' => 'bankusername为空'];
            }

            if (!$bankinfo['cardnum']) {
                return ['code' => 0, 'info' => 'cardnum为空'];
            }

            if (!$bankinfo['tel']) {
                return ['code' => 0, 'info' => 'tel为空'];
            }

            if (!$bankinfo['ifsc']) {
                return ['code' => 0, 'info' => 'ifsc为空'];
            }
